...
export function DashboardScreen() {
  const { projects, loading, syncError } = useInterviewStore();
  return (
    <div className="pb-20">
      ...
      <main className="space-y-3 p-4">
        ...
        {loading ? <Card><p className="text-sm text-slate-500">Syncing projects from Supabase…</p></Card> : null}
        {syncError ? <Card><p className="text-sm text-amber-700">{syncError}</p></Card> : null}
    ...
    <div>
      ...
      <main className="space-y-3 p-4">
        ...
        <Button
          ...
          onClick={async () => {
            if (!name.trim()) return;
            const project = await createNewProject(name.trim(), description.trim());
      ...
      <main className="space-y-3 p-4 text-sm">
        <Card className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Link to={`/projects/${project.id}/interview`}><Button variant="secondary" className="w-full" onClick={async () => setMode(project.id, 'discovery')}>Start Discovery</Button></Link>
            <Link to={`/projects/${project.id}/features`}><Button variant="secondary" className="w-full" onClick={async () => setMode(project.id, 'feature')}>Add Feature</Button></Link>
            <Link to={`/projects/${project.id}/roadmap`}><Button variant="ghost" className="w-full" onClick={async () => setMode(project.id, 'prioritization')}>Prioritize</Button></Link>
            <Button variant="ghost" className="w-full" onClick={async () => saveVersion(project.id, 'Saved from overview quick action.')}>Save Version</Button>
            <Button variant="primary" className="col-span-2 w-full" onClick={async () => { await generateHandoff(project.id); await setMode(project.id, 'handoff'); }}>Generate Handoff</Button>
          </div>
        </Card>
      ...
      <main className="space-y-3 p-4">
        <Card className="space-y-3">
          ...
          <div className="grid grid-cols-5 gap-1 text-[11px]">
            {(['discovery', 'feature', 'prioritization', 'versioning', 'handoff'] as const).map((mode) => (
              <Button key={mode} variant={project.activeMode === mode ? 'secondary' : 'ghost'} className="px-2" onClick={async () => setMode(project.id, mode)}>{mode.slice(0, 4)}</Button>
            ))}
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <button key={chip} onClick={async () => sendChat(project.id, chip)} className="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs text-slate-600">{chip}</button>
          ))}
        </div>

        <Card className="space-y-2">
          <Input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Share details, decisions, or scope changes..." />
          <Button className="w-full" onClick={async () => { if (input.trim()) { await sendChat(project.id, input.trim()); setInput(''); } }}>Send</Button>
        </Card>
    ...
    <div className="pb-16">
      ...
      <main className="space-y-3 p-4">
        <Button className="w-full" onClick={async () => {
          await setMode(project.id, 'feature');
    ...
    <div className="pb-16">
      ...
      <main className="space-y-3 p-4 text-sm">
        ...
        <Card className="space-y-2">
          ...
          <Button className="w-full" onClick={async () => { await setMode(project.id, 'feature'); navigate(`/projects/${project.id}/interview`); }}>Refine with AI</Button>
          <Button variant="secondary" className="w-full" onClick={async () => updateFeature(project.id, feature.id, { priority: 'P0' })}>Reprioritize to P0</Button>
...
export function RoadmapScreen() {
  ...
  return (
    <div className="pb-16">
      ...
      <main className="space-y-3 p-4">
        <Button variant="secondary" className="w-full" onClick={async () => setMode(project.id, 'prioritization')}>Enter Prioritization Mode</Button>
        {lanes.map((lane) => (
          <Card key={lane} className="space-y-2">
            <SectionTitle label={lane} />
                ...
                <div key={featureId} className="rounded-xl border border-slate-100 p-3 text-sm">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {lanes.filter((next) => next !== lane).map((nextLane) => (
                      <button key={nextLane} onClick={async () => moveRoadmapItem(project.id, featureId, nextLane)} className="rounded-full bg-slate-100 px-2 py-1 text-[11px]">
                        Move to {nextLane}
                      </button>
                    ))}
                  </div>
...
export function VersionHistoryScreen() {
  ...
  return (
    <div className="pb-16">
      ...
      <main className="space-y-3 p-4">
        <Button className="w-full" onClick={async () => { await setMode(project.id, 'versioning'); await saveVersion(project.id, 'Checkpoint created after product updates.'); }}>Save Version</Button>
        {project.versions.map((version) => (
          <Card key={version.id} className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button variant="ghost" onClick={async () => restoreVersion(project.id, version.id)}>Restore Version</Button>
            </div>
          </Card>
        ))}
      </main>
...
export function DecisionLogScreen() {
  ...
  return (
    <div className="pb-16">
      ...
      <main className="space-y-3 p-4">
        <Button className="w-full" onClick={async () => addDecision(project.id, 'Prioritize onboarding simplification', 'Reduce first-run friction in discovery mode.', 'Higher activation in week one.')}>+ Add Decision</Button>
...
export function HandoffScreen() {
  ...
  return (
    <div className="pb-16">
      ...
      <main className="space-y-3 p-4 text-sm">
        <Button className="w-full" onClick={async () => { await setMode(project.id, 'handoff'); await generateHandoff(project.id); }}>Refresh Handoff</Button>
    ...
    <div className="pb-16">
      ...
      <main className="space-y-3 p-4">
        <Card className="space-y-2">
          ...
          <Button onClick={async () => renameProject(project.id, name.trim(), description.trim())}>Save</Button>
        </Card>
        <Card className="space-y-2">
          <Button variant="ghost" onClick={async () => resetInterview(project.id)}>Reset Interview</Button>
          <Button variant="danger" onClick={async () => { await deleteProject(project.id); navigate('/projects'); }}>Delete Project</Button>
        </Card>
      </main>
    </div>
  );
